...
class QueryService:
    ...
    def __init__(self, ticket_api, ticket_parser, train_classifier, logger, query_history, notification_manager, config_manager=None):
        """
        初始化查询服务
        :param ticket_api: TicketAPI实例
        :param ticket_parser: TicketParser实例
        :param train_classifier: TrainClassifier实例
        :param logger: 日志记录器
        :param query_history: 查询历史记录器
        :param notification_manager: 通知管理器
        :param config_manager: 配置管理器（用于D/C识别模式）
        """
    ...
    def execute_query(self, date: str, from_station: str, to_station: str,
                     target_trains: List[str] = None, filters: Dict = None) -> Dict:
        """
        执行完整查询流程
        :param date: 出发日期
        ...
        if raw_data == "STATION_NOT_FOUND":
            return {"error": "STATION_NOT_FOUND", "table": "", "tickets": [], "all_tickets": []}
        if raw_data is None:
            return {"error": "QUERY_FAILED", "table": "", "tickets": [], "all_tickets": []}
        ...
        # 解析数据 - 使用return_table=True获取表格字符串
